...
// Filter Your Name only
wallpapers.forEach(item => {
  if (item.tags.includes('Your Name')) {
...
function renderCard(wallpaper, grid) {
  ...
  if (!localStorage.getItem(storageKey)) {
    localStorage.setItem(storageKey, randomRange(2000, 4000));
  }
  if (!localStorage.getItem(viewsKey)) {
    localStorage.setItem(viewsKey, randomRange(5000, 10000));
  }
